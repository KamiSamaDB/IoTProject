...
All four processes need to run simultaneously, each in its own terminal.
...
### Terminal 3 — Lighting publisher

**Windows**
```powershell
.venv\Scripts\python lighting.py
```

**macOS / Linux**
```bash
.venv/bin/python lighting.py
```
...
**Access Permissions table** — rows are publishers (thermostat, camera, lighting), columns are subscribers (localmonitor, insurance, fire department). Each cell is a toggle button. Clicking it grants or revokes that subscriber's ability to receive messages from that publisher in real time.
...
- Each subscriber party (localmonitor, insurance, fire department) is represented by a dedicated `paho-mqtt` client in the backend. When access to a publisher is granted, the client subscribes to that topic wildcard; when revoked, it unsubscribes immediately.
...
To change the MQTT broker or device ID, edit the constants at the top of `backend.py`, `thermostat.py`, `camera.py`, and `lighting.py`:
...
Subscriber keys use `snake_case` — the frontend automatically formats them as title case for display (e.g. `fire_department` → **Fire Department**). Both the permission matrix and the feed selector update automatically on next page load.
